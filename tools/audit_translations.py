...
PLUGIN_DIR = sync_translations.PLUGIN_DIR
...
def scan_unwrapped_ui_strings():
    ...
    for root, _, files in os.walk(SOURCE_DIR):
        if 'languages' in root or 'tools' in root or 'tests' in root:
            continue
        for file in files:
            if file.endswith('.lua') and file != 'localization_backup.lua':
                ...
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for pat in UI_WIDGET_PATTERNS:
                        for m in re.finditer(pat, content):
                            val = m.group(1)
                            # Ignore icons, font files, numbers, single chars, format templates, URLs, and punctuation separators
                            v_clean = val.strip()
...
def run_audit():
    print("==================================================")
    print("      BACKUP TRANSLATION & UI STRING AUDITOR      ")
    print("==================================================\n")
