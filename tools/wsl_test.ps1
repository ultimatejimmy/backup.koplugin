...
function Run-Workflow {
    ...
    # 2. Translation Audit
    Write-Host "Auditing translations..." -NoNewline
    $auditResult = python -X utf8 $AuditScript
    if ($LASTEXITCODE -ne 0) {
        Write-Host " FAILED" -ForegroundColor Red
    ...
    # Sync files to KOReader's plugin folder in WSL (exclude git and test artifacts)
    wsl rsync -rv --delete --exclude=".git/" --exclude="*.log" --exclude="backups/" "$WslPluginDir/" "$WSLDest/"
