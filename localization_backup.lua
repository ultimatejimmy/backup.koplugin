-- Localization Manager for Device Backup & Restore Plugin (with .po support)
...
local Localization = {
    ...
    translations = {},
    available_languages = {},
    path = "plugins/backup.koplugin",
}

-- Simple, robust .po file parser in pure Lua
    ...
    for line in file:lines() do
        ...
        if not (line:match("^#") or line:match("^%s*$")) then
            ...
            if line:match('^msgid%s+"') then
                ...
                -- Use greedy match so escaped quotes inside msgid do not terminate early
            ...
            elseif line:match('^"') then
                ...
                if in_msgid and msgid then
                    msgid = msgid .. (continuation or "")
                elseif in_msgstr and msgstr then
                    msgstr = msgstr .. (continuation or "")
                end
            end
        end
    end
...
function Localization:detectSystemLanguage()
    ...
    -- Normalize language code format (e.g. "zh-cn" -> "zh_cn", "pt-br" -> "pt_br")
    lang = lang:gsub("-", "_")
...
function Localization:init(path)
    logger.info("Localization: Initializing Backup localization...")
...
function Localization:discoverLanguages()
    ...
    local default_languages = {
        "ar", "de", "en", "es", "fr", "hu", "id", "it",
        "ja", "ko", "nl", "pl", "pt_br", "ru", "sk", "sr",
        "tr", "uk", "zh_CN"
    }

    if not lfs then
        logger.warn("Localization: lfs not available, using fallback language discovery")
        self.available_languages = default_languages
        return
    end
    ...
    if #self.available_languages == 0 then
        self.available_languages = default_languages
    else
